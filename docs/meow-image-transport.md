# Meow image transport (G7) — verified decision
...
pipeline (red-team C12 / gate G7); it was spiked on the real VPS before any
ingest code was written.
...
The ingest tool (P4) records `mp_content_posts.image_path` as the **container**
path (`/app/data/meow-assets/drafts/...`), never a Mac path.

## Why this passes ValidateImagePath
The publish path's allowed image root is `<dataDir>/meow-assets` (wired in P3,
...
bytes intact. The spike file was removed; the `meow-assets` root remains for P4.
