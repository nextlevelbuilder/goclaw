...
#### V3 Redesign — Core Architecture (2026-02-15 - 2026-04-06)
Major architectural upgrade with 8-stage pipeline, 3-tier memory, knowledge vault, evolution engine, and orchestration framework.

**Pipeline & Agent Loop (v3)**
- **Stage-based pipeline**: 8 pluggable stages (context → history → prompt → think → act → observe → memory → summarize) replacing monolithic agent loop
- **Dual-mode gate**: Feature flag `v3_pipeline_enabled` (per-agent) controls v2/v3 execution path, enables smooth migration
- **Pipeline callbacks**: Extensible hook points (`ContextStage`, `PromptStage`, `ObserveStage`, etc.) for middleware and extensions
- **Token-aware context**: Tiktoken-based token counting with BPE encoding, L0/L1/L2 progressive memory loading based on token budget

**DomainEventBus & Consolidation (v3)**
- **Event-driven workers**: 3 consolidation workers (episodic summarizer, semantic indexer, dreaming) driven by DomainEventBus events
- **Reliable event delivery**: Worker pool with retry, dedup, graceful shutdown. Event persistence prevents message loss
- **Memory consolidation**: Session summaries → episodic store, KG updates → semantic layer, LLM dream synthesis from consolidation logs

**3-Tier Memory Architecture (v3)**
- **Working memory**: Conversation history (raw messages + cost). Lives in session, auto-loaded on session open
- **Episodic memory**: Session summaries + key topics + emotions. Persistent in `episodic_summaries` table. L1 auto-injected before act stage
- **Semantic memory**: Knowledge graph with temporal filtering. L2 loaded on-demand via `memory_expand` tool or search queries
- **AutoInjector**: L0 context auto-injected during ContextStage with token budget awareness. Prevents exceeding context limits

**Knowledge Vault (v3)**
- **Document registry**: Agent-scoped documents with [[wikilinks]] for cross-reference navigation
- **Hybrid search**: BM25 + vector similarity, query planning for best strategy selection
- **Unified search API**: `/v1/agents/{id}/vault/search` returns docs + wikilinks + snippets
- **FS sync**: Vault docs backed to workspace/vault/, supports hot-reload from disk
- **UI dashboard**: Vault page with interactive graph visualization of wikilinks

**Self-Evolution Engine (v3)**
- **Metrics collection**: Per-run metrics (tool latency, error rate, context efficiency, token costs)
- **Suggestion analysis**: Daily/weekly cron analyzes metrics, generates adaptation suggestions (prompt tweaks, tool order, resource allocation)
- **Auto-adapt guardrails**: 3-stage progression (suggest → discuss → apply). Metrics dashboard + manual review before auto-apply. Rollback on regression
- **LLM-driven suggestions**: Evolution system uses Claude to synthesize recommendations from raw metrics

**Orchestration & Task Delegation (v3)**
- **Delegate tool**: Inter-agent task spawning with agent_links, token-aware work distribution, progress tracking
- **3 delegation modes**: auto (lead auto-selects agent), explicit (caller specifies agent), manual (caller chooses from UI list)
- **Task state persistence**: DB-backed task tracking with status transitions (pending → in_progress → completed/failed)
- **Capability matrix**: agent_links include tool capability assertions, prevents delegating to under-equipped agents

**WorkspaceContext Resolver (v3)**
- **6 scenarios**: Lead team workspace, member DM workspace, group workspace, user workspace, sandbox workspace, archive workspace
- **Transparent path rewriting**: File tool requests auto-routed to correct scope via WorkspaceInterceptor
- **Cross-tenant isolation**: All queries tenant-scoped, prevents accidental data leaks
- **Desktop support**: SQLite-backed workspace context for Lite edition

**ModelRegistry & Provider Adapter (v3)**
- **Forward-compat resolver**: Model aliases + seeded providers (Claude 3.5 Sonnet, GPT-4o, Qwen 3.5, etc.) with graceful fallback
- **Pluggable adapters**: ProviderAdapter interface for Anthropic, OpenAI, DashScope, Codex. Enables custom LLM backends
- **Native streaming**: All providers support token streaming via ServerSentEvents

**Feature Flags & Configurations (v3)**
- **Per-agent v3 flags**: `v3_pipeline_enabled`, `v3_memory_enabled`, `v3_vault_enabled`, `v3_evolution_enabled`, `v3_orchestration_enabled`
- **Default configuration**: Standard edition defaults to v3 on, Lite defaults to v2 (disabled) due to resource limits
- **Graceful fallback**: v2 agent loop available as fallback if v3 stages encounter errors

**Request Middleware (v3)**
- **Composable request chain**: Cache check → service tier lookup → request guards → auth verification
- **Zero-alloc fast path**: Hot requests (health checks, cache hits) avoid allocations
- **Service tier gating**: Request routing based on tenant service tier (free, pro, enterprise)

**Store Layer & sqlx Migration**
- **13 store interfaces** with PostgreSQL + SQLite implementations: SessionStore, AgentStore, MemoryStore, EpisodicStore, VaultStore, EvolutionStore, etc.
- **sqlx adoption**: Migrated all store layers to sqlx StructScan for type-safe queries (Phase 1-4 complete)
- **DB tag audits**: Added explicit `db:""` struct tags across all scannable types to prevent sqlx mapper errors
- **Integration tests**: 70+ tests covering all 13 store interfaces, dual-DB validation (PG + SQLite)
