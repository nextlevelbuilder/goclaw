...
func builtinToolSeedData() []store.BuiltinToolDef {
	defs := []store.BuiltinToolDef{
		...
		{Name: "memory_backlinks", DisplayName: "Memory Backlinks", Description: "List every memory document that links into the given path via Obsidian wikilinks", Category: "memory", Enabled: true,
			Requires: []string{"memory"},
		},
