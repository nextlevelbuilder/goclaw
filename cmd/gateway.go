...
func runGateway() {
	...
	// Wire the sandbox for the "Log in with Claude" flow (runs setup-token in a
	// per-connection container). sandboxMgr may be nil → login endpoints 501.
	if agentsH != nil {
		agentsH.SetSandbox(sandboxMgr, cfg.Agents.Defaults.Workspace)
	}
	...
	// WhatsApp Cloud (official Meta API) uses the same public base to build the
	// callback URL we surface to the user; the route is mounted in StartGateway.
	whatsappcloud.ConfigureWebhook(cfg.Gateway.PublicWebhookBase)
