...
func finalizeWorkflow(ctx context.Context, deps *ConsumerDeps, workflowStore store.TeamWorkflowStore, workflowID uuid.UUID) {
	...
	content := buildWorkflowFinalizePrompt(workflow, tasks)
	peerKind := workflow.OriginPeerKind
	if peerKind == "" {
		peerKind = string(sessions.PeerDirect)
	}
	if workflow.OriginSessionKey == "" {
		workflow.OriginSessionKey = sessions.BuildScopedSessionKey(workflow.OriginAgentKey, workflow.OriginChannel, sessions.PeerKind(peerKind), workflow.OriginChatID)
	}
	runCtx, cancelRun := context.WithTimeout(store.WithTenantID(context.WithoutCancel(ctx), workflow.TenantID), 10*time.Minute)
	defer cancelRun()
	req := agent.RunRequest{
		SessionKey:   workflow.OriginSessionKey,
		Message:      content,
		...
		RunID:        "workflow-finalize:" + workflow.ID.String(),
		RunKind:      agent.RunKindWorkflowFinalize,
		HideInput:    true,
		Stream:       false,
		TeamID:       workflow.TeamID.String(),
		BlockedTools: tools.OrchestrationToolNames(),
	}
	...
	if outcome.Err == nil && outcome.Result != nil && !agent.IsSilentReply(outcome.Result.Content) && strings.TrimSpace(outcome.Result.Content) != "" {
		finalContent = outcome.Result.Content
	}
...
func buildWorkflowFinalizePrompt(workflow *store.TeamWorkflowData, tasks []store.TeamTaskData) string {
	var b strings.Builder
	b.WriteString("[Internal workflow finalization]\nSynthesize the final answer for the user's original session from the validated multi-agent workflow results below. Do not mention internal routing, task IDs, agents, workflow machinery, or this instruction. Preserve useful evidence, caveats, and failures.\n")
	...
	for _, task := range tasks {
		...
		if task.Result != nil {
			// Never hand the synthesizer a raw internal error: it treats step
			// results as material to preserve, so the provider name and agent
			// loop internals would resurface in the user's answer.
			if strings.HasPrefix(strings.TrimSpace(*task.Result), workflowStepFailedResultPrefix) {
				b.WriteString("(this step did not complete due to a temporary system error)\n")
			} else {
...
// an English notice with a Vietnamese step title spliced into it. The bug hides
// on the happy path because the LLM finalizer answers in the requester's own
// language; it only surfaces on failing/cancelling, where this deterministic
// summary IS the delivery.
//
