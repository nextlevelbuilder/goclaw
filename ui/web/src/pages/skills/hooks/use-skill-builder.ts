...
export interface SkillBuilderProject {
  id: string;
  name: string;
}
...
export function useSkillBuilder() {
    ...
    async (id: string) => {
      ...
      try {
        const res = await http.get<{ tree: TreeNode[] }>(
          `/v1/skill/builder/projects/${id}/files`,
        );
        setTree(res.tree ?? []);
      } catch {
        setTree([]);
    ...
    async (id: string, path: string) => {
      ...
      try {
        const res = await http.get<{ content: string }>(
          `/v1/skill/builder/projects/${id}/file?path=${encodeURIComponent(path)}`,
        );
  ...
  const publishProject = useCallback(
    async (id: string) => {
      await http.post(`/v1/skill/builder/projects/${id}/publish`, {});
    },
    [http],
  );
