...
export const credentialsSchema: Record<string, FieldDef[]> = {
  ...
  google_chat: [
    { key: "serviceAccountFile", label: "Service Account JSON Path", type: "text", required: true, placeholder: "/app/config/google-sa.json", help: "Path to Google service account key file inside the container" },
  ],
  zalo_oa: [
...
export const configSchema: Record<string, FieldDef[]> = {
  ...
  google_chat: [
    { key: "mode", label: "Connection Mode", type: "select", options: [{ value: "pubsub", label: "Pub/Sub Pull" }, { value: "webhook", label: "Webhook (Phase 2)" }], defaultValue: "pubsub" },
    { key: "project_id", label: "GCP Project ID", type: "text", required: true, placeholder: "my-gcp-project" },
    { key: "subscription_id", label: "Pub/Sub Subscription", type: "text", required: true, placeholder: "my-chat-sub", help: "Pub/Sub subscription for Google Chat events" },
    { key: "pull_interval_ms", label: "Pull Interval (ms)", type: "number", defaultValue: 3000, help: "Pub/Sub pull frequency" },
    { key: "bot_user", label: "Bot User Email", type: "text", placeholder: "bot@project.iam.gserviceaccount.com", help: "Service account email to filter self-messages" },
    ...
    { key: "require_mention", label: "Require @mention in groups", type: "boolean", defaultValue: true },
    { key: "history_limit", label: "Group History Limit", type: "number", defaultValue: 50, help: "Max pending group messages for context (0 = disabled)" },
    { key: "dm_stream", label: "DM Streaming", type: "boolean", defaultValue: true, help: "Progressively edit message as LLM generates (DMs)" },
    { key: "group_stream", label: "Group Streaming", type: "boolean", defaultValue: false, help: "Progressively edit message as LLM generates (groups)" },
    { key: "long_form_threshold", label: "Long-Form Threshold", type: "number", defaultValue: 3500, help: "Character count above which content is uploaded as a Drive file (0 = disabled)" },
    { key: "long_form_format", label: "Long-Form Format", type: "select", options: [{ value: "md", label: "Markdown (.md)" }, { value: "txt", label: "Plain text (.txt)" }], defaultValue: "md" },
    { key: "media_max_mb", label: "Max Media Size (MB)", type: "number", defaultValue: 20 },
    { key: "file_retention_days", label: "Drive File Retention (days)", type: "number", defaultValue: 7, help: "Auto-delete uploaded Drive files after N days (0 = keep forever)" },
    { key: "drive_permission", label: "Drive Permission", type: "select", options: [{ value: "domain", label: "Domain" }, { value: "anyone", label: "Anyone with link" }], defaultValue: "domain" },
    { key: "drive_domain", label: "Drive Domain", type: "text", placeholder: "example.com", help: "Domain for domain-scoped Drive sharing" },
    { key: "allow_from", label: "Allowed Users", type: "tags", help: "Google Chat user IDs or emails" },
    { key: "block_reply", label: "Block Reply", type: "select", options: blockReplyOptions, defaultValue: "inherit", help: "Deliver intermediate text during tool iterations" },
  ],
  zalo_oa: [
    { key: "dm_policy", label: "DM Policy", type: "select", options: dmPolicyOptions, defaultValue: "pairing" },
