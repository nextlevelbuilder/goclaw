...
export const credentialsSchema: Record<string, FieldDef[]> = {
  ...
  zalo_bot: [
    ...
    { key: "webhook_secret", label: "Webhook Secret", type: "password", showWhen: { key: "transport", value: "webhook" }, help: "Generated by bot.zapps.me when you set the webhook URL there. Paste it back here. Until set, the channel runs in bootstrap mode (acks Zalo's verification ping with HTTP 200 but drops events)." },
  ],
  zalo_oa: [
...
export const configSchema: Record<string, FieldDef[]> = {
  ...
  zalo_bot: [
    { key: "transport", label: "Connection Mode", type: "select", options: [{ value: "webhook", label: "Webhook (recommended)" }, { value: "polling", label: "Polling" }], defaultValue: "webhook", help: "Webhook is event-driven and lighter. Polling is the fallback when no public URL is available." },
  ...
  zalo_oa: [
    { key: "transport", label: "Connection Mode", type: "select", options: [{ value: "webhook", label: "Webhook (recommended)" }, { value: "polling", label: "Polling" }], defaultValue: "webhook", help: "Webhook is event-driven and lighter on the server. Polling fetches via listrecentchat on a timer." },
    ...
    { key: "quote_user_message", label: "Quote user message", type: "boolean", defaultValue: false, help: "Reply by quoting the user's last inbound message (Zalo's reply-to). Turn on for CS-style threaded replies." },
    { key: "block_reply", label: "Block Reply", type: "select", options: blockReplyOptions, defaultValue: "inherit", help: "Deliver intermediate text during tool iterations" },
  ],
  zalo_personal: [
