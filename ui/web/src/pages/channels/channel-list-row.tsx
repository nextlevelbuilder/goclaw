...
export function ChannelListRow({
  ...
  const statusLabel = (() => {
    switch (status?.state) {
      ...
      default:
        return instance.enabled && status?.running
          ? t("status.running")
          : t("status.stopped");
    }
  })();
  const dotClass = (() => {
    if (!instance.enabled) return "bg-muted-foreground/40";
    switch (status?.state) {
      case "healthy":
  ...
  const summaryHint =
    instance.enabled && status?.summary && status.state !== "healthy"
      ? status.summary
      : null;
  ...
  return (
    ...
    >
      ...
      <div className="hidden shrink-0 text-xs text-muted-foreground sm:block sm:w-16">
        {!instance.enabled ? t("disabled") : statusLabel}
      </div>
