...
type chatTeamWorkTestEmbedder struct {
	called bool
}

func (e *chatTeamWorkTestEmbedder) Name() string  { return "test-embedder" }
func (e *chatTeamWorkTestEmbedder) Model() string { return "test-embedding" }
func (e *chatTeamWorkTestEmbedder) Embed(context.Context, []string) ([][]float32, error) {
	e.called = true
...
func TestChatMethodsApplyTeamWorkGateDisablesOrchestrationOnClassifierParseFailure(t *testing.T) {
	...
	methods.SetTeamWorkClassification(
		...
		nil, &chatTeamWorkTestEmbedder{}, nil, nil, nil, nil, nil, nil,
	)
...
func TestChatMethodsApplyTeamWorkGateClassifiesWithMissingAuxiliaryStores(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing embedder", agentStore: &chatTeamWorkAgentStore{agent: &store.AgentData{BaseModel: store.BaseModel{ID: agentID}, TenantID: tenantID, AgentKey: "lead"}}},
		{name: "missing agent store", embedder: &chatTeamWorkTestEmbedder{}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			methods.SetTeamWorkClassification(
				...
				nil, tc.embedder, nil, nil, nil, nil, nil, nil,
			)
			out := methods.applyTeamWorkGate(
				store.WithTenantID(context.Background(), tenantID),
