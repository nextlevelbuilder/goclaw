...
// integrationClassifyProvider is a deterministic provider that serves BOTH roles
// for one real *agent.Loop: the Team Work classifier stages AND the agent turn.
// It dispatches on each classifier stage's system-prompt marker and returns a
// validated SELF decision (so the run proceeds as a plain agent turn). Any request
// whose system prompt matches NO classifier marker is the real agent turn and
// falls to the default branch, where it optionally signals/blocks and then returns
// the fixed assistant answer. classifyCount increments only on the intent-resolve
// stage — the first stage of every ClassifyWithLLM invocation — so it counts whole
// classification cycles for THIS loop, not individual internal stage calls.
...
func (p *integrationClassifyProvider) Chat(ctx context.Context, req providers.ChatRequest) (*providers.ChatResponse, error) {
	...
	case strings.Contains(system, "Resolve the current user message into a complete standalone request"):
		...
		return &providers.ChatResponse{Content: `{"standalone_request":"do the bounded task","relation":"new","user_intent":"do the task","inherited_scope":[],"requested_deliverables":["done"],"quality_requirements":[],"explicit_constraints":[],"ambiguities":[],"needs_clarification":false}`}, nil
