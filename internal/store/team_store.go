...
// WorkflowReplan is the already-backend-validated replacement plan committed by
// CommitWorkflowReplan. The store never accepts a raw model-supplied graph: the
// coordinator-recovery path builds and freezes the plan through the same
// BuildPlanConstraint pipeline as the initial expansion, then hands the store a
// canonical plan + hash + the concrete tasks for the NEW revision. ExpectedRevision
// is the optimistic-concurrency guard — the commit only lands if the workflow is
// still on that revision, so two racing replans cannot both win.
type WorkflowReplan struct {
...
type TeamWorkflowStore interface {
	...
	// CommitWorkflowReplan atomically supersedes the current plan revision with a
	// backend-validated replacement: it locks the workflow, bumps plan_revision,
	// stores the new canonical plan/hash, invalidates active old-revision attempts
	// and marks their non-terminal tasks stale (completed results stay immutable),
	// inserts the new-revision tasks, and returns the workflow to running.
	CommitWorkflowReplan(ctx context.Context, replan WorkflowReplan) (WorkflowActionResult, error)
	...
	// ApplyWorkflowAction is the single authoritative entry point for the six
	// operator/coordinator recovery transitions that carry no backend-built plan
	// (retry_blocked, request_revision, cancel_workflow, fail_workflow,
	// retry_expansion, retry_delivery). apply_replan is NOT handled here: it is
	// service-orchestrated via CommitWorkflowReplan because it must commit a
	// backend-frozen task list the guard deliberately cannot carry. The transition
	// opens a transaction, locks and re-reads the authoritative workflow (PG FOR
