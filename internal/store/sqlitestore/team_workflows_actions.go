...
// for the six operator/coordinator recovery actions that carry no backend-built
// plan (retry_blocked, request_revision, cancel_workflow, fail_workflow,
// retry_expansion, retry_delivery). apply_replan is handled separately via
// CommitWorkflowReplan because it must commit a backend-frozen task list. Each
// branch opens one transaction, reads the authoritative workflow row (and the
// target task for step-scoped actions), enforces the optimistic
...
func (s *SQLiteTeamStore) ApplyWorkflowAction(ctx context.Context, guard store.WorkflowActionGuard) (store.WorkflowActionResult, error) {
	...
	case store.WorkflowActionRequestRevision:
		...
		if canApply && statusOK && taskStatusOK {
			...
			if n != 1 {
				var (
					curWorkflowStatus string
					curRevision       int
				)
				err := tx.QueryRowContext(ctx, `SELECT status,plan_revision FROM team_workflows WHERE id=? AND tenant_id=? AND team_id=?`,
					guard.WorkflowID, tid, guard.TeamID).Scan(&curWorkflowStatus, &curRevision)
				if err != nil && !errors.Is(err, sql.ErrNoRows) {
					return store.WorkflowActionResult{}, err
				}
				if err == nil && curRevision == revision && curWorkflowStatus == store.TeamWorkflowStatusNeedsRevision {
					return s.finishSQLiteAction(ctx, tx, tid, guard, store.WorkflowActionAlreadyApplied)
				}
				return s.finishSQLiteAction(ctx, tx, tid, guard, store.WorkflowActionConflict)
			}
			if _, err := tx.ExecContext(ctx, `UPDATE team_tasks SET status=?,dispatch_token=NULL,dispatch_lease_until=NULL,locked_at=NULL,lock_expires_at=NULL,updated_at=?
