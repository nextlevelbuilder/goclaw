...
func TestSQLiteApplyWorkflowActionTransitions(t *testing.T) {
	...
	t.Run("request revision", func(t *testing.T) {
		...
		if _, err := teamStore.db.Exec(`UPDATE team_tasks SET status=?,blocker_reason='needs revision',escalation_status=?,escalation_attempt_count=3 WHERE id=?`, store.TeamTaskStatusBlocked, store.TeamTaskEscalationPending, blockedID); err != nil {
			t.Fatal(err)
		}
		if _, err := teamStore.db.Exec(`UPDATE team_tasks SET status=?,dispatch_token=?,dispatch_lease_until=?,locked_at=?,lock_expires_at=? WHERE id=?`, store.TeamTaskStatusInProgress, token, future, future, future, activeID); err != nil {
			t.Fatal(err)
		}
		guard := sqliteStepActionGuard(store.WorkflowActionRequestRevision, teamID, workflow.ID, blockedID, store.TeamWorkflowStatusRunning, store.TeamTaskStatusBlocked, "split the delivery step")
		result, err := teamStore.ApplyWorkflowAction(ctx, guard)
	...
	t.Run("apply replan", func(t *testing.T) {
		...
		workflow := sqliteMakeRunningWorkflow(t, teamStore, ctx, teamID, leadID, []store.TeamTaskData{
			{BaseModel: store.BaseModel{ID: doneID}, TeamID: teamID, Subject: "Done", Status: store.TeamTaskStatusCompleted, Result: sqliteRecoveryStrPtr("evidence"), OwnerAgentID: &workerID, WorkflowStepID: "gather", WorkflowKind: store.TeamWorkflowTaskKindWork},
		...
		if _, err := teamStore.db.Exec(`UPDATE team_tasks SET status=?,blocker_reason='needs new plan' WHERE id=?`, store.TeamTaskStatusBlocked, blockedID); err != nil {
			t.Fatal(err)
		}
		newTaskID := uuid.New()
...
func TestSQLiteWorkflowActionReplayConcurrencyMatrix(t *testing.T) {
	...
	for _, action := range store.AllWorkflowActions {
		t.Run(string(action), func(t *testing.T) {
			teamStore, ctx, leadID, workerID, teamID := sqliteRecoveryFixture(t)
			workflowID, commentTaskID, guard, commit := sqliteActionMatrixFixture(t, teamStore, ctx, leadID, workerID, teamID, action)
			for _, bad := range sqliteActionMatrixBadGuards(ctx, guard, action) {
				result, err := sqliteApplyMatrixAction(bad.ctx, teamStore, bad.guard, commit)
			...
			for i := 0; i < callers; i++ {
				...
				go func() {
					defer wg.Done()
					<-start
					result, err := sqliteApplyMatrixAction(ctx, teamStore, guard, commit)
					if err != nil {
						errs <- err
						return
					}
			...
			replayResult, err := sqliteApplyMatrixAction(ctx, teamStore, guard, commit)
			if err != nil || replayResult.Outcome != sqliteActionMatrixReplay(action) {
				t.Fatalf("post-state %s result=%+v error=%v, want %v", action, replayResult, err, sqliteActionMatrixReplay(action))
			}
...
func sqliteApplyMatrixAction(ctx context.Context, teamStore *SQLiteTeamStore, guard store.WorkflowActionGuard, commit *store.WorkflowReplan) (store.WorkflowActionResult, error) {
...
func sqliteActionMatrixReplay(action store.WorkflowAction) store.WorkflowActionOutcome {
...
func sqliteActionMatrixFixture(t *testing.T, teamStore *SQLiteTeamStore, ctx context.Context, leadID, workerID, teamID uuid.UUID, action store.WorkflowAction) (uuid.UUID, uuid.UUID, store.WorkflowActionGuard, *store.WorkflowReplan) {
	t.Helper()
	if action == store.WorkflowActionRetryExpansion {
		...
		return workflow.ID, audit.ID, sqliteAdminActionGuard(action, teamID, workflow.ID, workflow.Status, 1, "matrix retry expansion"), nil
	}
	taskID := uuid.New()
	...
	if action == store.WorkflowActionRetryDelivery {
		...
		return workflow.ID, taskID, sqliteAdminActionGuard(action, teamID, workflow.ID, store.TeamWorkflowStatusCompleted, 1, "matrix retry delivery"), nil
	}
	...
	case store.WorkflowActionRetryBlocked:
		return workflow.ID, taskID, sqliteStepActionGuard(action, teamID, workflow.ID, taskID, store.TeamWorkflowStatusRunning, store.TeamTaskStatusBlocked, "matrix retry"), nil
	case store.WorkflowActionRequestRevision:
		return workflow.ID, taskID, sqliteStepActionGuard(action, teamID, workflow.ID, taskID, store.TeamWorkflowStatusRunning, store.TeamTaskStatusBlocked, "matrix revision"), nil
	case store.WorkflowActionCancelWorkflow:
		return workflow.ID, taskID, sqliteAdminActionGuard(action, teamID, workflow.ID, store.TeamWorkflowStatusRunning, 1, "matrix cancel"), nil
	case store.WorkflowActionFailWorkflow:
		return workflow.ID, taskID, sqliteAdminActionGuard(action, teamID, workflow.ID, store.TeamWorkflowStatusRunning, 1, "matrix fail"), nil
	case store.WorkflowActionApplyReplan:
		if _, err := teamStore.db.Exec(`UPDATE team_workflows SET status=? WHERE id=?`, store.TeamWorkflowStatusNeedsRevision, workflow.ID); err != nil {
			t.Fatal(err)
		}
		plan := []byte(`{"schema_version":1,"goal":"matrix replan","steps":["replacement"]}`)
		guard := sqliteStepActionGuard(action, teamID, workflow.ID, taskID, store.TeamWorkflowStatusNeedsRevision, store.TeamTaskStatusBlocked, "matrix replan")
