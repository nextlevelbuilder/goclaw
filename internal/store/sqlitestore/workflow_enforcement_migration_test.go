...
func TestEnsureSchema_MigrationV60PopulatedWorkflowPredecessor(t *testing.T) {
	db := openTestDB(t)
	if err := EnsureSchema(db); err != nil {
		t.Fatalf("EnsureSchema: %v", err)
	}
	rebuildWorkflowTablesAsV60(t, db)
	...
	for _, column := range []string{
		...
		if exists {
			t.Fatalf("v60 predecessor unexpectedly contains team_tasks.%s", column)
		}
	}
	if got := tableSQL(t, db, "team_workflows"); strings.Contains(got, "needs_revision") || strings.Contains(got, "classification_audit_id") {
		t.Fatalf("team_workflows is not the v60 predecessor shape: %s", got)
	}
	...
	mustExec(t, db, `INSERT INTO tenants(id,name,slug,status,settings) VALUES(?, 'Tenant', 'workflow-v60', 'active', '{}')`, tenantID)
	...
	mustExec(t, db, `INSERT INTO team_workflows(
		...
		?,'lead',?,'lead','run-v60','session-v60','ws','chat-v60',
		'group','local-v60','user-v60','sender-v60','member',
	...
	if err := EnsureSchema(db); err != nil {
		t.Fatalf("EnsureSchema (v60 to v61): %v", err)
	}
...
func TestEnsureSchema_MigrationV60RejectsDuplicateActiveOwners(t *testing.T) {
	db := openTestDB(t)
	if err := EnsureSchema(db); err != nil {
		t.Fatalf("EnsureSchema: %v", err)
	}
	...
	mustExec(t, db, `INSERT INTO tenants(id,name,slug,status) VALUES(?,'Tenant','duplicate-owner-v60','active')`, tenantID)
	...
	if err := EnsureSchema(db); err == nil {
		t.Fatal("v60 migration silently accepted duplicate active workflow owners")
	}
	var version int
	...
	if version != 60 {
		t.Fatalf("schema version = %d after rejected migration, want 60", version)
	}
}

func rebuildWorkflowTablesAsV60(t *testing.T, db *sql.DB) {
	t.Helper()
	...
	if fkOn != 1 {
		t.Fatalf("PRAGMA foreign_keys = %d after v60 fixture rebuild, want 1", fkOn)
	}
}
