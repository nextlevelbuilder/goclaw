...
var migrations = map[int]string{
	// Version 60 → 61: Team Work enforcement deltas — attempt fencing, blocker/
	...
	// runs under foreign_keys=OFF (see EnsureSchema, v == 60): team_tasks.workflow_id
	...
	60: `-- Classifier audit trail (created first: team_workflows FKs into it).
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_team_tasks_active_owner ON team_tasks(tenant_id, owner_agent_id) WHERE workflow_kind = 'work' AND owner_agent_id IS NOT NULL AND status IN ('dispatching','in_progress');`,
	// Version 59 → 60: typed user-notification policy with workflow backfill.
	59: `ALTER TABLE team_tasks ADD COLUMN notification_policy VARCHAR(24) NOT NULL DEFAULT 'default' CHECK (notification_policy IN ('default','suppress_handoff','workflow_internal'));
UPDATE team_tasks SET notification_policy='workflow_internal' WHERE workflow_id IS NOT NULL AND workflow_kind IN ('audit','work');`,
	// Version 58 → 59: durable multi-agent workflows and typed task linkage.
	58: `CREATE TABLE IF NOT EXISTS team_workflows (
    id TEXT NOT NULL PRIMARY KEY,
    team_id TEXT NOT NULL REFERENCES agent_teams(id) ON DELETE CASCADE,
    tenant_id TEXT NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
...
func EnsureSchema(db *sql.DB) error {
	...
	if current < SchemaVersion {
		...
		for v := current; v < SchemaVersion; v++ {
			...
			// v60 → v61: rebuilds team_workflows; team_tasks.workflow_id FKs into it.
			needsFKOff := v == 25 || v == 60
...
// ADD COLUMN statements (v60 → v61) that are already present. The upstream
