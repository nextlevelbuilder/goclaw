...
func TestPGWorkflowActionReplayConcurrencyMatrix(t *testing.T) {
	...
	for _, action := range store.AllWorkflowActions {
		t.Run(string(action), func(t *testing.T) {
			teamStore, ctx, leadID, workerID, teamID := pgRecoveryFixture(t)
			workflowID, commentTaskID, guard := pgActionMatrixFixture(t, teamStore, ctx, leadID, workerID, teamID, action)
...
func pgActionMatrixFixture(t *testing.T, teamStore *PGTeamStore, ctx context.Context, leadID, workerID, teamID uuid.UUID, action store.WorkflowAction) (uuid.UUID, uuid.UUID, store.WorkflowActionGuard) {
	t.Helper()
	if action == store.WorkflowActionRetryExpansion {
		...
		return workflow.ID, audit.ID, pgAdminActionGuard(action, teamID, workflow.ID, workflow.Status, 1, "matrix retry expansion")
	}
	...
	if terminal {
		...
		return workflow.ID, taskID, pgAdminActionGuard(action, teamID, workflow.ID, store.TeamWorkflowStatusCompleted, 1, "matrix retry delivery")
	}
	...
	case store.WorkflowActionRetryBlocked:
		return workflow.ID, taskID, pgStepActionGuard(action, teamID, workflow.ID, taskID, store.TeamWorkflowStatusRunning, store.TeamTaskStatusBlocked, "matrix retry")
	case store.WorkflowActionCancelWorkflow:
		return workflow.ID, taskID, pgAdminActionGuard(action, teamID, workflow.ID, store.TeamWorkflowStatusRunning, 1, "matrix cancel")
	case store.WorkflowActionFailWorkflow:
		return workflow.ID, taskID, pgAdminActionGuard(action, teamID, workflow.ID, store.TeamWorkflowStatusRunning, 1, "matrix fail")
	}
