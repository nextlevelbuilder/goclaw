...
// re-reading the task's current token/status/revision. Called only when the CAS
// matched no rows, inside the same transaction so the read is consistent.
//
// The CAS matched zero rows for one of two reasons:
//
//  1. THIS attempt already performed the transition (idempotent replay — e.g. the
//     tool path called CompleteWorkflowTaskAttempt during the turn, then the
//     post-turn settlement in cmd/gateway_workflow_finalize.go replays it). The
//     transition's own UPDATE clears dispatch_token to NULL on success, so on
//     replay the token no longer matches the CAS — this path must classify the
//     replay as AlreadyApplied so settlement falls through to
//     DispatchUnblockedTasks instead of returning Stale, which would strand every
//     dependent behind a completed root (the live G4 DAG defect: roots completed,
//     terminals never unblocked because settle returned Stale on the replay).
//  2. A NEWER attempt (recovery/replan/requeue minted a different token, or a
//     newer plan_revision took ownership) superseded this attempt → Stale.
//
// The disambiguation is the ownership fingerprint. An attempt owns a task when
// either its live token still matches, or — once a terminal transition cleared
// the token to NULL — the plan_revision still matches and the task sits in a
// target state this attempt drove it to. A non-nil token different from this
// attempt's, or a plan_revision that moved past this attempt's, is a newer
// owner → Stale.
func probePGAttemptOutcome(ctx context.Context, q workflowRowScanner, attempt store.WorkflowTaskAttempt, targetStatuses ...string) store.WorkflowMutationOutcome {
	...
	// A newer plan revision means a replan superseded this attempt, regardless of
	// token state — the task now belongs to a different plan generation.
	if revision != attempt.PlanRevision {
		return store.WorkflowMutationStale
	}
	// A live, non-nil token that is NOT this attempt's means a newer attempt owns
	// the task right now.
	if token != nil && *token != attempt.DispatchToken {
		return store.WorkflowMutationStale
	}
	// token matches this attempt, OR token is nil (cleared by this attempt's own
	// prior terminal transition). Either way the revision still matches, so this
	// is the same attempt — classify as idempotent replay iff the task reached a
	// target state this transition produces.
...
func (s *PGTeamStore) CompleteWorkflowTaskAttempt(ctx context.Context, attempt store.WorkflowTaskAttempt, result string) (store.WorkflowTaskTransition, error) {
	...
	if n, _ := res.RowsAffected(); n == 1 {
		...
		// The 1-row branch above computed ReadyToFinalize; the idempotent replay
		// path must too, or settle skips finalizeWorkflow and strands the workflow
		// running with all tasks completed. Finalization is claim-guarded + idempotent.
