...
//
// input carries the canonical coordinator resolved from the durable roster so a
// coordinated (multi_role) route — executable or not — records the lead
// responsible for the decision in the CoordinatorAgentID column, even when the
// route path carries no Plan (and therefore no FinalOwnerAgentID).
func BuildClassificationAudit(in ClassificationAuditInput, result Result, input Input) *store.TeamWorkClassificationAudit {
	...
	setAuditOwners(audit, result, input)
	return audit
}
...
//
// The route path (result.Plan == nil) reaches a coordinated (multi_role) route
// through makeCoordinatedTeamResult, which carries the canonical lead in
// BestTeamOwnerID for the EXECUTABLE case but leaves it unset for the
// non-executable fail-closed case. The durable coordinator is input.CoordinatorAgentID
// (resolved from the roster before the classifier ran), so the audit records it
// in CoordinatorAgentID for every multi_role route — executable (where owner ==
// coordinator) and non-executable (where there is no owner because the work never
// runs). A single_owner route keeps BestTeamOwnerID as the selected owner and no
// coordinator. This keeps the coordinator column linkable to the agent
// responsible for the decision across the G7 fail-closed boundary.
func setAuditOwners(audit *store.TeamWorkClassificationAudit, result Result, input Input) {
	if result.Plan != nil {
	...
	if result.EffectiveWorkflowMode == WorkflowModeMultiRole {
		if input.CoordinatorAgentID != uuid.Nil {
			id := input.CoordinatorAgentID
			audit.CoordinatorAgentID = &id
		}
		// An executable coordinated route sets BestTeamOwnerID == coordinator; record
		// it as the selected owner so the owner column still resolves. A
		// non-executable route leaves BestTeamOwnerID unset (no owner runs), so only
		// the coordinator is recorded.
...
// degradedStageFromReason maps route and retained historical reason codes onto
// their originating stage so audits can group both current and persisted records.
