...
func TestPostgresWorkflowMigrationPopulatedUpgradeAndRollback(t *testing.T) {
	...
	if err := m.Steps(1); err != nil {
		t.Fatalf("apply workflow migration v96: %v", err)
	}
	assertPostgresMigrationVersion(t, db, 96, false)

	ids := seedPostgresWorkflowV96(t, db)
	if err := m.Steps(1); err != nil {
		t.Fatalf("apply notification-policy migration v97: %v", err)
	}
	assertPostgresTaskPolicy(t, db, ids.workflowTask, "workflow_internal")
	assertPostgresTaskPolicy(t, db, ids.genericTask, "default")

	if err := m.Steps(1); err != nil {
		t.Fatalf("apply enforcement migration v98: %v", err)
	}
	assertPostgresMigrationVersion(t, db, RequiredSchemaVersion, false)
	assertPostgresWorkflowEndpoint(t, db, ids)
	t.Log("populated PostgreSQL v95→v98 upgrade preserved and backfilled workflow state")

	// Compatible v98 state must make a clean one-step rollback and re-upgrade.
	if err := m.Steps(-1); err != nil {
		t.Fatalf("safe v98 to v97 rollback: %v", err)
	}
	...
	if err := m.Steps(1); err != nil {
		t.Fatalf("re-apply v98 after safe rollback: %v", err)
	}
	assertPostgresWorkflowEndpoint(t, db, ids)
	t.Log("safe PostgreSQL v98↔v97 round trip passed")

	// A lifecycle value that exists only in v98 must reject down migration. The
	...
	if _, err := db.Exec(`UPDATE team_workflows SET status='needs_revision' WHERE id=$1`, ids.workflow); err != nil {
		t.Fatalf("set v98-only status: %v", err)
	}
	if err := m.Steps(-1); err == nil {
		t.Fatal("v98 down migration accepted needs_revision state")
	}
	assertPostgresMigrationVersion(t, db, 97, true)
	...
	t.Log("v98-only workflow status rejected rollback and marked migration dirty")

	// Non-default task enforcement state also has no v97 representation.
	if _, err := db.Exec(`UPDATE team_tasks SET blocker_reason='waiting for operator' WHERE id=$1`, ids.workflowTask); err != nil {
		t.Fatalf("set v98-only task state: %v", err)
	}
	if err := m.Steps(-1); err == nil {
		t.Fatal("v98 down migration accepted blocker state")
	}
	assertPostgresMigrationVersion(t, db, 97, true)
	...
	t.Log("v98-only task state rejected rollback without data loss")

	// Append-only classifier audits cannot be represented at v97. The rollback
	...
	if err := m.Steps(-1); err == nil {
		t.Fatal("v98 down migration accepted classifier audit state")
	}
	assertPostgresMigrationVersion(t, db, 97, true)
	var auditCount int
	...
	// Revision 2 may reuse a step ID in v98. Restoring the predecessor index must
	...
	if err := m.Steps(-1); err == nil {
		t.Fatal("v98 down migration accepted duplicate workflow step across revisions")
	}
	assertPostgresMigrationVersion(t, db, 97, true)
...
func TestPostgresWorkflowMigrationRejectsDuplicateActiveOwners(t *testing.T) {
	...
	if err := m.Migrate(96); err != nil {
		t.Fatalf("migrate isolated schema to v96: %v", err)
	}
	ids := seedPostgresWorkflowV96(t, db)
	if _, err := db.Exec(`INSERT INTO team_tasks(
		id,team_id,subject,status,owner_agent_id,metadata,task_type,
	...
	if err := m.Migrate(98); err == nil {
		t.Fatal("v98 migration silently accepted duplicate active workflow owners")
	}
	...
	t.Log("duplicate active owners aborted PostgreSQL v98 migration without choosing a winner")
}
...
func seedPostgresWorkflowV96(t *testing.T, db *sql.DB) postgresWorkflowIDs {
	t.Helper()
	...
	if _, err := db.Exec(`INSERT INTO team_workflows(
		...
		$5,'lead',$5,'lead','run-v96','session-v96','ws','chat-v96',
		'group','local-v96','user-v96','sender-v96','member',
...
func assertPostgresWorkflowEndpoint(t *testing.T, db *sql.DB, ids postgresWorkflowIDs) {
	...
	for index, fragments := range map[string][]string{
		...
		var indexDef string
		if err := db.QueryRow(`SELECT pg_get_indexdef(to_regclass($1))`, index).Scan(&indexDef); err != nil {
	...
	if err := db.QueryRow(`SELECT to_regclass('team_work_classification_audits') IS NOT NULL`).Scan(&auditTableExists); err != nil {
