...
func TestBuildCodexPoolActivitySeparatesDirectSelectionAndFailover(t *testing.T) {
	...
	if !last.UsedFailover {
		t.Fatal("last.UsedFailover = false, want true")
	}
	if last.SelectedProvider != "pool-a" || last.ProviderName != "pool-b" {
		t.Fatalf("last routing = selected %q provider %q", last.SelectedProvider, last.ProviderName)
	}
	if last.AttemptCount != 2 {
		t.Fatalf("last.AttemptCount = %d, want 2", last.AttemptCount)
	}
}
