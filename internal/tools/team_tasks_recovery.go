...
func (t *TeamTasksTool) applyWorkflowRecoveryAction(
	...
	if action == store.WorkflowActionApplyReplan {
		return t.manager.ApplyWorkflowReplan(ctx, WorkflowReplanRequest{
			Team: target.team, Workflow: target.workflow, Blocked: target.blocked,
			Guard: guard, CoordinatorID: target.agentID,
		})
	}
...
func (t *TeamTasksTool) executeRequestRevision(ctx context.Context, args map[string]any) *Result {
	reason, err := workflowRecoveryText(args, "request_revision")
	if err != nil {
		return ErrorResult(err.Error())
	}
	target, err := t.resolveWorkflowRecoveryTarget(ctx)
	...
	result, err := t.applyWorkflowRecoveryAction(ctx, target, store.WorkflowActionRequestRevision, reason)
	if err != nil {
		return ErrorResult("failed to request workflow revision: " + err.Error())
	}
	if result.Conflict() {
		return workflowActionConflictResult(store.WorkflowActionRequestRevision)
	}
	if !result.Applied() {
		return NewResult("Workflow is already paused for revision.")
	}
	return NewResult("Workflow is paused for revision. Review the blocked step and call apply_replan with the requirements for the replacement plan.")
}

func (t *TeamTasksTool) executeApplyReplan(ctx context.Context, args map[string]any) *Result {
	reason, err := workflowRecoveryText(args, "apply_replan")
	if err != nil {
		return ErrorResult(err.Error())
	}
	target, err := t.resolveWorkflowRecoveryTarget(ctx)
	...
	result, err := t.applyWorkflowRecoveryAction(ctx, target, store.WorkflowActionApplyReplan, reason)
	if err != nil {
		return ErrorResult("failed to apply workflow replan: " + err.Error())
	}
	if result.Conflict() {
		return workflowActionConflictResult(store.WorkflowActionApplyReplan)
	}
	if !result.Applied() {
		return NewResult("Workflow replan was already applied.")
	}
	if t.workflowActionService() == nil {
		if dispatcher, ok := t.manager.(PostTurnProcessor); ok {
			dispatcher.DispatchUnblockedTasks(ctx, target.team.ID)
	...
	return NewResult(fmt.Sprintf("Workflow replacement plan applied at revision %d.", result.Workflow.PlanRevision))
}
