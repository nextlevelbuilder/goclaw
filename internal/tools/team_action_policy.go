...
// the bounded coordinator recovery actions for existing workflow records:
// retry_blocked, cancel_workflow, and fail_workflow. retry_expansion and
// retry_delivery are deliberately NOT here — they are admin-RPC-only
...
// reject, attach, ask_user, clear_ask_user. The bounded coordinator actions are
// exposed for cross-edition parity. retry_expansion and retry_delivery remain
// admin-RPC-only in both editions.
