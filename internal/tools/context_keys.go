...
const (
	...
	ctxTeamWorkReviewRequired     toolContextKey = "team_work_review_required"
...
// WithTeamWorkReviewRequired carries the classifier's review-required decision to create_dag. False for non-coordinated directives.
func WithTeamWorkReviewRequired(ctx context.Context, required bool) context.Context {
	return context.WithValue(ctx, ctxTeamWorkReviewRequired, required)
}

// TeamWorkReviewRequiredFromCtx returns the gate's review-required flag; false when unset (back-compat).
func TeamWorkReviewRequiredFromCtx(ctx context.Context) bool {
	v, _ := ctx.Value(ctxTeamWorkReviewRequired).(bool)
	return v
}
