...
type ProviderConfig struct {
	...
	// OAuthToken is a subscription (Pro/Max/Team/Enterprise) OAuth token, as
	// produced by `claude setup-token`. Only Anthropic uses it today — it lets
	// the connected Claude Code CLI (delegate_external) authenticate against a
	// Claude subscription instead of a metered API key. Env-only, never in
	// config.json (see config_secrets.go).
