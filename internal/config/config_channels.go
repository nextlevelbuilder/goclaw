...
type GatewayConfig struct {
	...
	TeamWorkClassifyTimeoutSec int                 `json:"team_work_classify_timeout_sec,omitempty"` // per-call Team Work LLM deadline in seconds, covering the single routing classifier call and the agent loop's directive enforcement (0/unset = built-in 30s route, 30s enforcement). Classification is ONE LLM call and needs no embedding provider, but a slow agent model still needs a larger value or the route fails safe to a degraded self and enforcement discards the validated plan. Capped at 300.
