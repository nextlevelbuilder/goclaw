...
type MattermostConfig struct {
	Enabled        bool                `json:"enabled"`
	ServerURL      string              `json:"server_url"`          // Mattermost server URL (e.g. "https://mm.example.com")
	BotToken       string              `json:"bot_token"`           // Bot access token or Personal Access Token
	TeamName       string              `json:"team_name,omitempty"` // default team name (optional — auto-detected if bot is in one team)
	AllowFrom      FlexibleStringSlice `json:"allow_from"`
	DMPolicy       string              `json:"dm_policy,omitempty"`       // "open" (default), "allowlist", "disabled"
	GroupPolicy    string              `json:"group_policy,omitempty"`    // "open" (default), "allowlist", "disabled"
	RequireMention *bool               `json:"require_mention,omitempty"` // require @bot mention in channels (default true)
	...
	BlockReply     *bool               `json:"block_reply,omitempty"`     // override gateway block_reply (nil = inherit)
	DebounceDelay  int                 `json:"debounce_delay,omitempty"`  // ms delay before dispatching rapid messages (default 300, 0=disabled)
	ThreadTTL      *int                `json:"thread_ttl,omitempty"`      // hours before thread participation expires (default 24, 0=disabled)
	MediaMaxBytes  int64               `json:"media_max_bytes,omitempty"` // max file download size in bytes (default 50MB)
}
