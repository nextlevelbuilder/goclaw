...
// maxClassifyTimeoutSec bounds the configurable per-call Team Work LLM deadline.
// The value applies to the single classifier routing call and to each
// directive-enforcement call, so an unbounded value could hold a chat turn (and
// its scheduler slot) for minutes before the gate resolves. 300s per call is
// already far beyond any healthy provider.
...
type Settings struct {
	...
	// pipeline: the single classifier routing call AND each of the agent loop's
	// directive-enforcement calls. Zero means "use the package defaults" (30s
	// route in internal/teamworkclassify, 30s enforcement in internal/agent). It
	// exists because those defaults are hard-coded constants while model latency
	// is a per-agent property: an agent whose runtime model needs ~30s per turn
	// degrades when a call crosses the line, so the routing decision fails safe to
	// a degraded self — or worse, the enforcement call dies and a validated plan is
	// discarded. Bounded by maxClassifyTimeoutSec.
...
type Defaults struct {
	...
	ClassifierProvider string
	ClassifierModel    string
	// ClassifyTimeoutSec is the file-config per-call Team Work LLM deadline in
