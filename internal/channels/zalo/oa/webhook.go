...
func (c *Channel) HandleWebhookEvent(ctx context.Context, raw json.RawMessage) error {
	...
	if c.creds().OAID == "" {
		n := c.bootstrapDroppedCount.Add(1)
		if n == 1 {
			slog.Warn("zalo_oa.webhook.preconsent_drop",
				"instance_id", c.instanceID,
				"hint", "complete Zalo OA consent before processing inbound events")
		} else {
			slog.Debug("zalo_oa.webhook.preconsent_drop", "instance_id", c.instanceID)
		}
		return nil
	}
...
func (c *Channel) dispatchWebhookText(ctx context.Context, e *oaInboundEvent) {
	...
	c.HandleAuthorizedMessage(e.Sender.ID, e.Sender.ID, e.Message.Text, nil, metadata, "direct")
}
