...
// Webhook signature scheme (official Zalo doc):
//   X-ZEvent-Signature = "mac=" + hex(SHA256(appID + rawBody + timestamp + secret))
//
// The verifier accepts both the documented "mac=<hex>" form and the bare
// hex form (observed traffic), comparing digests in constant time.
//
...
// SignatureMode controls verifier behavior. Empty (unset) → disabled, which
// keeps polling-transport channels (no public inbound endpoint at all) and
// the pre-secret bootstrap gate in channel.go's inBootstrap() working as
// designed: New() already forces an unset mode to strict for webhook
// transport, so "" only reaches here for transports that never enforce a
// signature in the first place. An unrecognized *non-empty* value — a typo
// or stale enum member surviving in persisted config — is different: that
// can only happen to a webhook-transport channel whose operator already
// chose real enforcement, so it must fail closed (strict) rather than
// silently falling back to accepting unsigned payloads on a public
// endpoint.
...
func normalizeMode(m string) string {
	switch {
	case m == SignatureModeStrict, m == SignatureModeLogOnly, m == SignatureModeDisabled:
		return m
	case m == "":
...
func (v *oaSignatureVerifier) Verify(headers http.Header, body []byte) error {
	...
	// Zalo's official doc transmits the digest as "mac=<hex>". Strip the
	// prefix if present; accept both forms so observed-traffic drift can't
	// lock us out. The digest itself is still verified in constant time.
	sig = strings.TrimPrefix(sig, "mac=")
