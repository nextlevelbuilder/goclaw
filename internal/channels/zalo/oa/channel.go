...
func New(name string, cfg config.ZaloOAConfig, creds *ChannelCreds,
	ciStore store.ChannelInstanceStore, msgBus *bus.MessageBus, pairingSvc store.PairingStore) (*Channel, error) {
	...
	if cfg.Transport == "webhook" && strings.TrimSpace(cfg.WebhookSignatureMode) == "" {
		cfg.WebhookSignatureMode = string(SignatureModeStrict)
	}
	if cfg.DMPolicy == "" {
	...
	c.SetPairingService(pairingSvc)
	c.ValidatePolicy(cfg.DMPolicy, "")
	return c, nil
}

// SetInstanceID propagates the DB instance ID to the token source so
// Persist / cursor flush / router registration use the real UUID.
func (c *Channel) SetInstanceID(id uuid.UUID) {
	c.instanceID = id
	c.tokens.instanceID = id
}

// SetTenantID propagates tenant scope to BaseChannel + token source.
...
// "webhook" (default) registers with the shared router even before OAuth
// consent so the create wizard can complete Zalo's URL verification.
// "polling" starts the listrecentchat poll loop after consent.
func (c *Channel) Start(_ context.Context) error {
	...
	if c.creds().OAID == "" {
		switch transport {
		case "webhook":
			if err := c.startWebhookTransport(); err != nil {
				return err
			}
			if c.resolvedSlug == "" {
				return nil
		...
		case "polling":
			// Polling needs an OAuth access token and starts only after consent.
		default:
		...
		slog.Info("zalo_oa.started", "state", "unauthorized", "transport", transport, "name", c.Name())
		c.MarkDegraded("awaiting consent", "complete Zalo OA consent after channel setup",
...
// sends one attachment per CS message, so multiple Media entries are
// delivered sequentially in order. Caption of each file follows that
// attachment; msg.Content trails the last attachment. Returns ErrPartialSend
// if an earlier attachment succeeded but a later item or trailing text failed.
func (c *Channel) Send(ctx context.Context, msg bus.OutboundMessage) error {
	...
	for i, m := range msg.Media {
		last := i == len(msg.Media)-1
		attachMID, unsupported, err := c.deliverAttachment(ctx, msg.ChatID, m)
		if err != nil {
			if delivered {
		...
		if unsupported {
			note := i18n.T(store.LocaleFromContext(ctx), i18n.MsgZaloOAUnsupportedAttachment,
				filepath.Base(m.URL), m.ContentType)
			fallback := note
			if last {
				body := mergeTrailingText(m.Caption, msg.Content)
				if body != "" {
					fallback = body + "\n\n" + note
				}
			} else if m.Caption != "" {
			...
			if _, terr := c.SendText(ctx, msg.ChatID, fallback, ""); terr != nil {
				if delivered {
			...
			delivered = true
			c.recordReplyLen(msg.ChatID, len(fallback))
			continue
		}
		delivered = true
		trailing := m.Caption
...
// deliverAttachment uploads and sends one media item. unsupported=true means
// the MIME is outside Zalo OA's image/gif/PDF/DOC/DOCX set; the caller should
// send a text note instead of failing the whole batch.
func (c *Channel) deliverAttachment(ctx context.Context, chatID string, m bus.MediaAttachment) (attachMID string, unsupported bool, err error) {
	...
	if mt == "image/gif" {
		...
		if len(data) > zaloGIFCapBytes {
			return "", false, fmt.Errorf("zalo_oa: gif too large: %d bytes (Zalo cap is 5MB)", len(data))
		}
		mid, sendErr := c.SendGIF(ctx, chatID, data)
		return mid, false, sendErr
	}
	if strings.HasPrefix(mt, "image/") {
		...
		mid, sendErr := c.SendImage(ctx, chatID, compressed, newMT)
		return mid, false, sendErr
	}
	...
	if len(data) > zaloFileCapBytes {
		return "", false, fmt.Errorf("zalo_oa: file too large: %d bytes (Zalo cap is 5MB)", len(data))
	}
	mid, sendErr := c.SendFile(ctx, chatID, data, filepath.Base(m.URL))
	return mid, false, sendErr
}
