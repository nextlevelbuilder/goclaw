...
func (c *Channel) OnReactionEvent(ctx context.Context, chatID, messageID, status string) error {
	...
	if status == "done" || status == "error" {
		...
		rc.tombstoneOnce.Do(func() {
			// Hold rc.mu across the stopCh check + Add. Stop() takes rc.mu
			// per controller (Range → rc.Stop()) before reactionWG.Wait(),
			// so an Add here either completes before Stop's Range reaches
			// this controller (Wait then blocks correctly) or sees a closed
			// stopCh and never Adds. Without the lock, Add can land after
			// Wait returned → WaitGroup misuse panic.
			rc.mu.Lock()
			defer rc.mu.Unlock()
			select {
			case <-c.stopCh:
