...
func (c *Channel) dispatchWebhookMedia(parent context.Context, e *oaInboundEvent, forceImageKind bool) {
	...
	if !c.authorizeInbound(parent, e.Sender.ID, e.Sender.ID) {
		return
	}
	...
	c.HandleAuthorizedMessage(e.Sender.ID, e.Sender.ID, content, []string{path}, metadata, "direct")
}
...
func (c *Channel) dispatchWebhookLink(ctx context.Context, e *oaInboundEvent) {
	if e.Sender.ID == "" {
		return
	}
	att := firstAttachment(e.Message.Attachments)
	if att == nil || att.Payload.URL == "" {
		if strings.TrimSpace(e.Message.Text) != "" {
			c.dispatchWebhookText(ctx, e)
		}
		return
	}
	if !c.authorizeInbound(ctx, e.Sender.ID, e.Sender.ID) {
		return
	}
	...
	c.HandleAuthorizedMessage(e.Sender.ID, e.Sender.ID, b.String(), nil, metadata, "direct")
}

const oaWebhookMaxMediaBytes = 20 * 1024 * 1024

// downloadOAMedia fetches an attachment under SSRF guard + 20MB cap.
// dewee has no tools.NewSSRFSafeClient; host pinning is done by
// tools.CheckSSRF before the request, with a bounded client for the fetch.
func downloadOAMedia(ctx context.Context, fileURL string) (string, error) {
	...
	client := &http.Client{Timeout: 60 * time.Second} // ctx governs deadline
	...
	tmpFile, err := os.CreateTemp("", "dewee_zoa_*"+ext)
	if err != nil {
