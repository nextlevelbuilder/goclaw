...
type Router struct {
	...
	setupSlugs     map[string]time.Time
	dedup          *Dedup
...
func NewRouter() *Router {
	return &Router{
		...
		setupSlugs:     make(map[string]time.Time),
...
// RegisterSetupSlug temporarily primes a not-yet-created channel slug so the
// Zalo Developers console can verify its webhook URL before the create request
// persists credentials. Registered channel instances supersede setup probes.
func (r *Router) RegisterSetupSlug(slug string) error {
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	if _, registered := r.slugToInstance[slug]; registered {
		return nil
	}
	for existing, expiresAt := range r.setupSlugs {
		if !expiresAt.After(now) {
			delete(r.setupSlugs, existing)
		}
	}
	r.setupSlugs[slug] = now.Add(defaultSetupSlugTTL)
	return nil
}

func (r *Router) setupSlugActive(slug string) bool {
	now := r.now()
	r.mu.Lock()
	defer r.mu.Unlock()
	expiresAt, ok := r.setupSlugs[slug]
	if !ok {
		return false
	}
	if !expiresAt.After(now) {
		delete(r.setupSlugs, slug)
		return false
	}
	return true
}
...
func (r *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		if r.setupSlugActive(suffix) {
			slog.Info("zalo_webhook.setup_probe_acknowledged",
				"slug", suffix, "remote", req.RemoteAddr)
	...
	body, err := io.ReadAll(io.LimitReader(req.Body, r.maxBodySize+1))
	if err != nil {
	...
	if !ok || resolvedID != instanceID || resolvedInst != inst {
		...
		if ok && resolvedInst != nil {
			resolvedInst.dispatchWG.Done()
		}
