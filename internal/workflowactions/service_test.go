...
func TestServiceApplyPublishesAndDispatchesOnlyApplied(t *testing.T) {
	...
	service := New(storeStub, storeStub, msgBus, nil, dispatcher)
...
func TestServiceActionPublicationAndDispatchMatrix(t *testing.T) {
	...
	for _, action := range store.AllWorkflowActions {
		t.Run(string(action), func(t *testing.T) {
			...
			service := New(storeStub, storeStub, msgBus, func(context.Context, ReplanRequest) (store.WorkflowActionResult, error) { return storeStub.apply, nil }, dispatcher)
			...
			wantDispatch := action == store.WorkflowActionRetryBlocked || action == store.WorkflowActionApplyReplan
...
func TestServiceApplyReplanFailuresAndConflictsDoNotPublishOrDispatch(t *testing.T) {
	tenantID, teamID, workflowID, taskID := uuid.New(), uuid.New(), uuid.New(), uuid.New()
	workflow := &store.TeamWorkflowData{
		BaseModel: store.BaseModel{ID: workflowID}, TenantID: tenantID, TeamID: teamID,
		Status: store.TeamWorkflowStatusNeedsRevision, PlanRevision: 3,
	}
	tasks := []store.TeamTaskData{{
		BaseModel: store.BaseModel{ID: taskID}, TenantID: tenantID, TeamID: teamID,
		Status: store.TeamTaskStatusBlocked, WorkflowKind: store.TeamWorkflowTaskKindWork,
		WorkflowID: &workflowID, WorkflowStepID: "blocked", PlanRevision: 3,
	}}
	ctx := store.WithTenantID(context.Background(), tenantID)
	msgBus := bus.New()
	dispatcher := &workflowActionTestDispatcher{}
	...
	guard := workflowActionTestGuard(teamID, workflowID, taskID)
	guard.Action = store.WorkflowActionApplyReplan
	guard.ExpectedStatus = store.TeamWorkflowStatusNeedsRevision
	guard.ExpectedPlanRevision = 3
	plannerErr := errors.New("planner failed closed")
	service := New(storeStub, storeStub, msgBus, func(context.Context, ReplanRequest) (store.WorkflowActionResult, error) {
		return store.WorkflowActionResult{}, plannerErr
	}, dispatcher)
	if _, err := service.Apply(ctx, guard); !errors.Is(err, plannerErr) {
		t.Fatalf("planner failure error=%v, want %v", err, plannerErr)
	}
	if len(events) != 0 || dispatcher.calls != 0 {
		t.Fatalf("planner failure emitted events=%d dispatches=%d", len(events), dispatcher.calls)
	}

	service.replan = func(context.Context, ReplanRequest) (store.WorkflowActionResult, error) {
		return store.WorkflowActionResult{Outcome: store.WorkflowActionConflict, Action: store.WorkflowActionApplyReplan, Workflow: workflow}, nil
	}
	result, err := service.Apply(ctx, guard)
	if err != nil || result.Outcome != store.WorkflowActionConflict || result.Workflow != workflow || len(result.Tasks) != 1 || result.Tasks[0].ID != taskID {
		t.Fatalf("replan conflict result=%+v error=%v", result, err)
	}
	if len(events) != 0 || dispatcher.calls != 0 {
		t.Fatalf("replan conflict emitted events=%d dispatches=%d", len(events), dispatcher.calls)
	}
}
...
func TestServiceWorkflowUpdatedEventSerializesWithoutSecrets(t *testing.T) {
	...
	service := New(nil, nil, msgBus, nil, nil)
	service.publish(ctx, store.WorkflowActionResult{
		Outcome: store.WorkflowActionApplied, Action: store.WorkflowActionApplyReplan, Workflow: workflow,
	})
	...
	if payload.TenantID != tenantID.String() || payload.TeamID != teamID.String() || payload.WorkflowID != workflowID.String() || payload.Action != string(store.WorkflowActionApplyReplan) || payload.Outcome != "applied" {
...
func TestAllowedActionsAuthoritativeMatrix(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			tasks:    blocked,
			want:     []store.WorkflowAction{store.WorkflowActionRetryBlocked, store.WorkflowActionRequestRevision, store.WorkflowActionCancelWorkflow, store.WorkflowActionFailWorkflow},
		},
		{
			name:     "needs revision blocked",
			workflow: store.TeamWorkflowData{Status: store.TeamWorkflowStatusNeedsRevision, PlanRevision: 2},
			tasks:    blocked,
			want:     []store.WorkflowAction{store.WorkflowActionRetryBlocked, store.WorkflowActionApplyReplan, store.WorkflowActionCancelWorkflow, store.WorkflowActionFailWorkflow},
		},
		{
