...
func init() {
	register(LocaleZH, map[string]string{
		...
		// Zalo OA 渠道
		MsgZaloOAUnsupportedAttachment: "Zalo OA 不支持附件 %s（%s）；只能发送图片、GIF 和 PDF/DOC/DOCX 文件。",
		MsgZaloOAErrAuth:               "Zalo 拒绝访问令牌（%d: %s）。渠道将自动刷新并重试一次。",
		MsgZaloOAErrRefreshExpired:     "Zalo 刷新令牌已过期（%d: %s）；请重新授权 OA 以恢复发送。",
		MsgZaloOAErrSize:               "附件超过 Zalo 限制（图片 1MB、文件 5MB、GIF 5MB）。",
		MsgZaloOAErrPermission:         "OA 应用缺少此调用所需的扩展权限。",
		MsgZaloOAErrUserNotVisible:     "该用户对此 OA 不可见（未订阅或已隐藏资料）。",
		MsgZaloOAErrAppDisabled:        "Zalo 应用已被禁用或封禁；请联系 Zalo 支持。",
		MsgZaloOAErrInteractionWindow:  "Zalo 仅允许向近期与 OA 互动过的用户发送消息。",
		MsgZaloOAErrRate:               "此 OA 或用户的 Zalo 配额已用尽；请等待配额窗口重置。",
		MsgZaloOAErrServer:             "Zalo 返回临时服务器错误；短暂退避后重试是安全的。",
		MsgZaloOAErrPayload:            "Zalo 拒绝请求载荷；请检查消息结构和接收者 ID 格式。",
		MsgZaloOAErrRedirectURI:        "Zalo 拒绝 OAuth redirect_uri；请更新 Zalo 控制台以匹配渠道配置。",
		MsgZaloOAReauthDueSoon:         "即将需要重新授权：刷新令牌约 %d 天后过期。",
		MsgZaloOACodeExchangeFailed:    "Zalo OAuth 授权码交换失败：%s",
		MsgZaloOAInvalidChannelType:    "实例不是 zalo_oa 渠道",
		MsgZaloOAConnected:             "Zalo 公众号已连接：%s",
		MsgZaloOAInvalidState:          "OAuth state 令牌无效或已过期；请重新点击 Connect",
		MsgZaloOARedirectURIRequired:   "credentials.redirect_uri 为必填，且必须与开发者控制台的回调一致",
		MsgZaloOAMissingAppID:          "credentials.app_id 为必填（请求授权 URL 前请先填写）",
		MsgZaloOAStateGenFailed:        "生成 state 令牌失败",
		MsgZaloOACallbackUnavailable:   "暂时无法确定网关的公开 URL — 请先通过公开域名打开一次控制台",
		MsgZaloOAAuthUnavailable:       "Zalo OA 授权功能当前不可用",
		MsgZaloOAOAIDMismatch:          "回调 OA 与实例 OA 不一致 — 请粘贴本实例授权页面上的 URL",
	})
}
