...
func init() {
	register(LocaleZH, map[string]string{
		...
		MsgMeowNotAuthorized:  "❌ 无权操作。仅限所有者 — 请先在已配置的所有者聊天中运行 /meow verify。",
		MsgMeowOpsUnavailable: "此实例未启用 Meow 内容命令。",
		MsgMeowBadDate:        "日期无效 %q（需要 YYYY-MM-DD）。",
		MsgMeowChannelUnknown: "未知频道 %s。",
		MsgMeowError:          "错误：%s",
		MsgMeowQueueUsage:     "用法：/meow queue <@handle>",
		MsgMeowQueueEmpty:     "%s 没有草稿或已批准的帖子。",
		MsgMeowQueueHeader:    "📋 %s 的队列：",
		MsgMeowPreviewUsage:   "用法：/meow preview <@handle> <YYYY-MM-DD>",
		MsgMeowPostNone:       "%s 在 %s 没有帖子。",
		MsgMeowApproveUsage:   "用法：/meow approve <@handle> <YYYY-MM-DD>",
		MsgMeowApproved:       "✅ 已批准 %s 在 %s 的帖子。",
		MsgMeowNoDraftToAct:   "%s 在 %s 没有草稿。",
		MsgMeowSkipUsage:      "用法：/meow skip <@handle> <YYYY-MM-DD>",
		MsgMeowSkipped:        "✅ 已跳过 %s 在 %s 的帖子。",
		MsgMeowNoPostToSkip:   "%s 在 %s 没有草稿或已批准的帖子。",
		MsgMeowEditUsage:      "用法：/meow edit <@handle> <YYYY-MM-DD> ko|en <文本>",
		MsgMeowEditBadField:   "字段必须为 'ko' 或 'en'（收到 %q）。",
		MsgMeowEdited:         "✅ 已更新 %s 文案，频道 %s，日期 %s。",
		MsgMeowIngestUsage:    "用法：/meow ingest <收件箱内的 bundle.json>",
		MsgMeowIngestHeld:     "⚠️ 已保留（未导入）：%s",
		MsgMeowIngested:       "✅ 已为 %s 在 %s 导入草稿。",
	})
}
