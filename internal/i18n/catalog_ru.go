...
func init() {
	register(LocaleRU, map[string]string{
		...
		MsgAbortForced:          "выполнение принудительно прервано (превышена отсрочка 3 с)",
		...
		MsgToolBrowser:            "Автоматизировать взаимодействие с браузером: переходить по страницам, кликать элементы, заполнять формы, делать скриншоты",
		...
		MsgToolCron:               "Планировать или управлять повторяющимися задачами с помощью cron-выражений, времени или интервалов",
		...
		MsgUpdateCacheStale:     "Кэш обновлений устарел; выполните обновление перед применением обновления",
