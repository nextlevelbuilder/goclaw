...
func init() {
	register(LocaleEN, map[string]string{
		...
		// Zalo OA channel
		MsgZaloOAUnsupportedAttachment: "Attachment %s (%s) is not supported by Zalo OA; only images, GIFs, and PDF/DOC/DOCX files can be sent.",
		MsgZaloOAErrAuth:               "Zalo access token rejected (%d: %s). The channel refreshes and retries once automatically.",
		MsgZaloOAErrRefreshExpired:     "Zalo refresh token has expired (%d: %s); re-authorize the OA to resume sending.",
		MsgZaloOAErrSize:               "Attachment exceeds the Zalo cap (image 1MB, file 5MB, gif 5MB).",
		MsgZaloOAErrPermission:         "The OA app is missing an extended permission required for this call.",
		MsgZaloOAErrUserNotVisible:     "The target user is not visible to this OA (not opted-in or profile hidden).",
		MsgZaloOAErrAppDisabled:        "The Zalo app is disabled or banned; contact Zalo support.",
		MsgZaloOAErrInteractionWindow:  "Zalo only allows messaging users who interacted with the OA recently.",
		MsgZaloOAErrRate:               "Zalo quota for this OA or user has been exhausted; wait for the quota window to reset.",
		MsgZaloOAErrServer:             "Zalo returned a temporary server error; retrying after a short backoff is safe.",
		MsgZaloOAErrPayload:            "Zalo rejected the request payload; verify message shape and recipient ID format.",
		MsgZaloOAErrRedirectURI:        "Zalo rejected the OAuth redirect_uri; update the Zalo console to match the channel config.",
		MsgZaloOAReauthDueSoon:         "Re-consent due soon: the refresh token expires in about %d day(s).",
		...
		MsgZaloOAInvalidState:          "oauth state token is invalid or expired; start Connect again",
		MsgZaloOARedirectURIRequired:   "credentials.redirect_uri is required and must match the dev-console callback",
		MsgZaloOAMissingAppID:          "credentials.app_id is required (set it on the channel before requesting consent URL)",
		MsgZaloOAStateGenFailed:        "failed to generate state token",
		MsgZaloOACallbackUnavailable:   "cannot determine the gateway public URL yet — open the dashboard once via its public domain",
		MsgZaloOAAuthUnavailable:       "Zalo OA authorization is unavailable",
		MsgZaloOAOAIDMismatch:          "callback OA differs from instance OA — paste the URL from THIS instance’s consent page",
	})
}
