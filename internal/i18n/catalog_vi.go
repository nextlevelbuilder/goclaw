...
func init() {
	register(LocaleVI, map[string]string{
		...
		// Kênh Zalo OA
		MsgZaloOAUnsupportedAttachment: "Tệp đính kèm %s (%s) không được Zalo OA hỗ trợ; chỉ gửi được ảnh, GIF và tệp PDF/DOC/DOCX.",
		MsgZaloOAErrAuth:               "Zalo từ chối access token (%d: %s). Kênh sẽ tự làm mới và thử lại một lần.",
		MsgZaloOAErrRefreshExpired:     "Refresh token Zalo đã hết hạn (%d: %s); hãy cấp quyền lại cho OA để tiếp tục gửi.",
		MsgZaloOAErrSize:               "Tệp đính kèm vượt giới hạn Zalo (ảnh 1MB, tệp 5MB, gif 5MB).",
		MsgZaloOAErrPermission:         "Ứng dụng OA thiếu quyền mở rộng cần thiết cho lệnh gọi này.",
		MsgZaloOAErrUserNotVisible:     "Người nhận không hiển thị với OA này (chưa opt-in hoặc đã ẩn hồ sơ).",
		MsgZaloOAErrAppDisabled:        "Ứng dụng Zalo đã bị vô hiệu hóa hoặc khóa; hãy liên hệ Zalo support.",
		MsgZaloOAErrInteractionWindow:  "Zalo chỉ cho phép nhắn người dùng đã tương tác gần đây với OA.",
		MsgZaloOAErrRate:               "Đã hết hạn mức Zalo cho OA hoặc người dùng này; chờ chu kỳ quota đặt lại.",
		MsgZaloOAErrServer:             "Zalo trả về lỗi máy chủ tạm thời; thử lại sau một khoảng lùi ngắn là an toàn.",
		MsgZaloOAErrPayload:            "Zalo từ chối payload; kiểm tra cấu trúc tin nhắn và định dạng ID người nhận.",
		MsgZaloOAErrRedirectURI:        "Zalo từ chối redirect_uri OAuth; cập nhật Zalo console khớp với cấu hình kênh.",
		MsgZaloOAReauthDueSoon:         "Sắp đến hạn cấp quyền lại: refresh token hết hạn sau khoảng %d ngày.",
		MsgZaloOACodeExchangeFailed:    "đổi mã oauth zalo thất bại: %s",
		MsgZaloOAInvalidChannelType:    "instance không phải kênh zalo_oa",
		MsgZaloOAConnected:             "đã kết nối zalo official account: %s",
		MsgZaloOAInvalidState:          "state token oauth không hợp lệ hoặc đã hết hạn; hãy bấm Connect lại",
		MsgZaloOARedirectURIRequired:   "credentials.redirect_uri là bắt buộc và phải khớp callback đăng ký trên dev-console",
		MsgZaloOAMissingAppID:          "credentials.app_id là bắt buộc (điền trước khi lấy URL consent)",
		MsgZaloOAStateGenFailed:        "không tạo được state token",
		MsgZaloOACallbackUnavailable:   "chưa xác định được public URL của gateway — hãy mở dashboard một lần qua domain công khai",
		MsgZaloOAAuthUnavailable:       "Chức năng cấp quyền Zalo OA hiện không khả dụng",
		MsgZaloOAOAIDMismatch:          "OA từ callback khác OA của instance — hãy dán URL từ trang consent của CHÍNH instance này",
	})
}
