...
func (l *Loop) buildPipelineDeps(req *RunRequest, bridgeRS *runState) pipeline.PipelineDeps {
	...
	recoveryCfg := pipeline.DefaultRecoveryConfig()
	...
	return pipeline.PipelineDeps{
		...
		Config: pipeline.PipelineConfig{
			...
			Truncation:         ptr(pipeline.DefaultTruncationConfig()),
			Microcompact:       ptr(pipeline.DefaultMicrocompactConfig()),
			ContextCollapse:    pipeline.DefaultCollapser(),
			MaxToolConcurrency: 10,
			StreamingToolExec:  req.Stream,
		...
		IsToolConcurrencySafe: func(tc providers.ToolCall) bool {
			...
			meta := reg.GetMetadata(tc.Name)
			tool, ok := reg.Get(tc.Name)
			if !ok {
				return meta.IsReadOnly()
			}
			classifier, _ := tool.(tools.ConcurrencyClassifier)
			return tools.IsConcurrencySafeForTool(tc.Name, meta, classifier, tc.Arguments)
		},
		...
		SessionEndHooks:  cb.sessionEndHooks,
	}
}
...
func convertRunInput(req *RunRequest) *pipeline.RunInput {
	return &pipeline.RunInput{
		...
		SelfKnowledgeExplain: classifySelfKnowledgeExplainIntent(req.Message),
	}
}
