		...
		if slices.Contains(cfg.ToolNames, "delegate_external") && slices.Contains(cfg.ToolNames, "github_publish_dir") {
			lines = append(lines,
				"## Porting or refactoring a whole repository (→ one PR)\n"+
					...
					"The DEFAULT procedure fans the port out across several coding-agent runs that work AT THE SAME TIME — it is much faster and is what the user wants. Pass a distinct `worker` to each `delegate_external` so each gets its OWN sandbox (they share the workspace). Do EXACTLY this unless the repo is tiny (see fallback at the end):\n"+
					"1. SETUP — ONE `delegate_external`, `worker`=\"setup\". Instruct it to: clone the PUBLIC repo into the workspace; create the target Go module skeleton (`port/go.mod` + shared types packages under `port/`); and produce a plan that splits the source into 2–3 chunks that are as INDEPENDENT as possible (each = packages that mostly don't depend on the others). Report the plan + the workspace-relative paths. It must NOT push or open a PR.\n"+
					"2. PORT IN PARALLEL — in ONE message, issue 2–3 `delegate_external` calls TOGETHER, each with a distinct `worker` (\"1\",\"2\",\"3\"). Each ports its assigned chunk from the shared clone into its own packages under `port/` and builds just those packages. Use 2–3 workers (host memory caps it at ~3). Sending them in the SAME message is the whole point — they run simultaneously; NEVER issue them one at a time. None push or open a PR.\n"+
					...
					"4. PUBLISH — `github_publish_dir` (owner, repo, source_dir = `port/`, branch, commit_message, pr_title). It commits every file and opens the PR through the user's connected GitHub — no PAT — and returns the PR URL. Report that URL.\n"+
						"\n"+
