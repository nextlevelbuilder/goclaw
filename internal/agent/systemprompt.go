		...
		if slices.Contains(cfg.ToolNames, "delegate_external") && slices.Contains(cfg.ToolNames, "github_publish_dir") {
			...
			lines = append(lines,
				"## Porting or refactoring a whole repository (→ one PR)\n"+
					...
					"1. SETUP — ONE `delegate_external`, `worker`=\"setup\". KEEP THIS STEP FAST (target: a couple of minutes). It is a cheap mechanical scaffold + split plan, NOT porting and NOT a code review. Instruct it, IN THESE WORDS, to: FIRST `rm -rf port go-port` (leftovers from a previous run), then clone the PUBLIC repo FRESH; create ONLY `port/go.mod` plus a single small `port/shared/` package holding the handful of cross-cutting type definitions; and write `port/PLAN.md` assigning every source file to a chunk. CRITICAL efficiency rules to pass along VERBATIM: \"Build the plan from CHEAP SIGNALS ONLY — the output of `find`/`ls` and `wc -l`, plus each file's import/`use` lines (`grep`). Do NOT read whole source files, do NOT summarize their logic, and do NOT create a stub file per source file. Creating hundreds of placeholder files, or reading the whole codebase to plan, is the single slowest mistake you can make here — each porting worker creates its OWN files in the next step. Your entire output is: go.mod, a small shared/ package, and PLAN.md.\" Tell it to split into about "+ns+" chunks — exactly how many workers run AT ONCE on this host, so ~"+ns+" balanced chunks finish the port in ONE parallel wave. Make chunks as INDEPENDENT and EQUALLY-SIZED as possible (group by directory/module, balanced by total line count). Do NOT over-split: more than ~"+ns+" chunks does NOT go faster — the extras just queue into later waves. It reports the plan + workspace-relative paths, and must NOT push or open a PR. (If it comes back having ported real logic anyway, STILL continue to step 2 — do not publish from setup output alone.)\n"+
					"2. PORT IN PARALLEL — in ONE message, issue ONE `delegate_external` call per chunk from the plan (about "+ns+"), ALL TOGETHER, each with a distinct `worker` (\"1\",\"2\",\"3\",…). Each WRITES the Go implementation for ITS chunk (creating its own files under `port/`) by translating the Rust sources listed for that chunk. Pass each worker this instruction VERBATIM: \"Write the code and stop. Do NOT run `go build`/`go vet`/`go test` and do NOT iterate on compiler errors — your chunk references packages other workers are still writing, so it CANNOT compile yet and a build loop here just burns time. `gofmt` your files, then report what you wrote. A single integration pass compiles everything afterwards.\" The platform runs ~"+ns+" at once (the rest queue), so keeping the plan to ~"+ns+" chunks means a single fast wave. Sending them in the SAME message is the whole point — NEVER issue them one at a time. None push or open a PR. You MUST reach this step — a whole-repo port is not done until the parallel workers have run.\n"+
