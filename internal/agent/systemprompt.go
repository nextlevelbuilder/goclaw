		...
		if slices.Contains(cfg.ToolNames, "delegate_external") && slices.Contains(cfg.ToolNames, "github_publish_dir") {
			lines = append(lines,
				"## Porting or refactoring a whole repository (→ one PR)\n"+
					...
					"1. SETUP — ONE `delegate_external`, `worker`=\"setup\". Its job is ONLY scaffolding + a split plan, NOT porting. Instruct it, IN THESE WORDS, to: clone the PUBLIC repo into the workspace; create the target Go module skeleton (`port/go.mod` + empty package directories + shared type definitions under `port/`); and write a PLAN file (`port/PLAN.md`) that assigns every source file/package to a chunk. Tell it to use AS MANY chunks as the code naturally splits into by independence — one chunk per group of packages that don't depend on the others — up to about 8 chunks; more, smaller, independent chunks are BETTER (they balance the parallel workers). CRITICAL — tell it explicitly: \"Do NOT translate or implement any business logic. Do NOT fill in function bodies. Leave every ported function as an empty stub / `// TODO(worker N)` placeholder. The actual porting is done by other agents in the next step; if you port it all yourself you defeat the whole point.\" It reports the plan + the workspace-relative paths, and must NOT push or open a PR. (If the setup agent comes back having ported real logic anyway, STILL continue to step 2 for any chunk it left as stubs — do not publish from setup output alone.)\n"+
					"2. PORT IN PARALLEL — in ONE message, issue ONE `delegate_external` call per chunk from the plan, ALL TOGETHER, each with a distinct `worker` (\"1\",\"2\",\"3\",…). Each fills in the actual Go implementation for ITS chunk's stubs from the shared Rust clone, and builds just its own packages. Issue as many as the plan has chunks (up to ~8) — you do NOT need to throttle for host capacity; the platform automatically runs as many as the host can handle at once and queues the rest. Sending them in the SAME message is the whole point — NEVER issue them one at a time. None push or open a PR. You MUST reach this step — a whole-repo port is not done until the parallel workers have run.\n"+
