...
func buildTeamMD(team *store.TeamData, members []store.TeamMemberData, selfID uuid.UUID, isV2 bool) string {
	...
	if selfRole == store.TeamRoleLead {
		if isV2 {
			...
			sb.WriteString("- **Check task board first** — call `team_tasks(action=\"list\")` before creating tasks. The system blocks creation if you skip this step\n")
			sb.WriteString("- Create all tasks first, then briefly announce what was delegated and to whom — then STOP\n")
			...
			sb.WriteString("1. Identify distinct deliverables (research, writing, design, code...)\n")
			sb.WriteString("2. Create 1 task per deliverable — assign by member skill, use short actionable subjects\n")
			sb.WriteString("3. Create independent tasks FIRST → get their UUIDs from the response → THEN create dependent tasks with `blocked_by=[UUID]`\n")
			...
			sb.WriteString("Plan the full task graph upfront — do NOT wait for results to create follow-up tasks.\n")
