...
// TestStoreVault_Search_SharedDocsIncluded verifies that shared vault documents
// (agent_id IS NULL) appear in agent-filtered search results.
// Regression test for GH#917: AND agent_id = $N excluded NULL rows.
func TestStoreVault_Search_SharedDocsIncluded(t *testing.T) {
	db := testDB(t)
	tenantID, agentID := seedTenantAgent(t, db)
	ctx := tenantCtx(tenantID)
	vs := newVaultStore(db)
	...
	// Insert shared doc (agent_id = NULL) — must be visible to any agent's search.
	// Title carries the unique term so both PG FTS and SQLite LIKE paths can match it.
	sharedDoc := &store.VaultDocument{
		TenantID:    tid,
		AgentID:     nil, // shared: no agent owner — do NOT use makeVaultDoc here
		Scope:       "shared",
		Path:        "shared/917-regression.md",
		Title:       "XTerm917SharedRegressionUnique",
		DocType:     "note",
		ContentHash: "hash-shared-917",
		Summary:     "Shared document visible to all agents",
	}
	if err := vs.UpsertDocument(ctx, sharedDoc); err != nil {
		t.Fatalf("UpsertDocument shared: %v", err)
	}

	// Insert agent-owned doc for baseline — makeVaultDoc sets AgentID correctly here.
	agentDoc := makeVaultDoc(tid, aid, "agent/917-owned.md", "XTerm917AgentOwnedUnique")
	agentDoc.Summary = "Agent-owned document"
	if err := vs.UpsertDocument(ctx, agentDoc); err != nil {
		t.Fatalf("UpsertDocument agent: %v", err)
	}

	t.Run("shared_doc_found_by_agent_search", func(t *testing.T) {
		results, err := vs.Search(ctx, store.VaultSearchOptions{
			Query:      "XTerm917SharedRegressionUnique",
			TenantID:   tid,
			AgentID:    aid, // agent filter set — shared doc must still appear
			MaxResults: 10,
		})
		if err != nil {
			t.Fatalf("Search: %v", err)
		...
		found := false
		for _, r := range results {
			if r.Document.Path == "shared/917-regression.md" {
				found = true
			}
		}
		if !found {
			t.Errorf("shared doc not found in agent search results (got %d results)", len(results))
		}
	})

	t.Run("agent_doc_still_found", func(t *testing.T) {
		results, err := vs.Search(ctx, store.VaultSearchOptions{
			Query:      "XTerm917AgentOwnedUnique",
			TenantID:   tid,
			AgentID:    aid,
			MaxResults: 10,
		})
		...
		found := false
		for _, r := range results {
			if r.Document.Path == "agent/917-owned.md" {
				found = true
			}
		}
		if !found {
			t.Errorf("agent-owned doc not found after fix (regression: %d results)", len(results))
		}
	})
}
